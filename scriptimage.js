...
// let city = "Huixquilucan"; // Reemplaza "Ciudad" con el nombre de la ciudad que desees

document.getElementById("cityForm").addEventListener("submit", function(event) {
    event.preventDefault(); // Evita que se recargue la página al enviar el formulario
    const city = document.getElementById("cityInput").value; // Obtiene el valor ingresado en el input
    generateRandomOutfit(city); // Llama a la función `generateRandomOutfit` con el valor de la ciudad
...
function generateRandomOutfit(city) {
    const API_URL = `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${apiKey}&units=metric`;
    ...
    fetch(API_URL)
      ...
      .then(data => {
        ...
        getPexelsImage(description)
        .then(imageUrl => {
          displayOutfit(temperature, description, outfit, imageUrl);
        })
        .catch(error => {
          console.log('Error al obtener imagen de Pexels:', error);
          displayOutfit(temperature, description, outfit, null);
        });
    })
    .catch(error => {
...
async function getPexelsImage(description) {
  const pexelsAPI_URL = `https://api.pexels.com/v1/search?query=${description}&per_page=1`;

  const response = await fetch(pexelsAPI_URL, {
        headers: {
            Authorization: 'Bearer wVXVXS3O69qsmW0M63O3zyMh3fNWPuYuIf9UjnpEmmA9ZCSnt3ZnxkCb'
        }
    });
...
"Maxi vestido y sandalias de plataforma"
  ...
  function generateOutfitNublado() {
    const outfitsNublado = [
      ...
      "Suéter ligero, jeans ajustados y botas",
"Camisa, pantalones cortos y zapatillas blancas",
"Chaqueta vaquera, vestido ajustado y botines",
"Sudadera delgada con capucha, leggings y zapatillas deportivas",
"Blusa estampada, pantalones de tela y mocasines",
"Chaleco, camiseta básica y jeans",
"Blazer, falda lápiz y tacones",
"Suéter de cuello V, pantalones de cuero y botas altas",
...
function displayOutfit(temperature, description, outfit, imageUrl) {
    const temperatureElement = document.getElementById('temperature');
    const descriptionElement = document.getElementById('description');
    const outfitElement = document.getElementById('outfit');
    const outfitImageElement = document.getElementById('imageUrl');
...
temperatureElement.textContent = 'Temperatura: ' + temperature + '°C';
  ...
  outfitImageElement.src = imageUrl;
  outfitImageElement.alt = 'Imagen del outfit';
  outfitImage.src = imageUrl;
if (imageUrl) {
    imageElement.src = imageUrl;
    imageElement.alt = "Imagen del clima";
